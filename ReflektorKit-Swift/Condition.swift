...
struct Condition: Hashable, Parsable {
    ...
    internal struct Expression: Hashable, Parsable {
        ...
        init(rawString: String) throws {
            ...
            self.rawString = Parser.normalizeExpressionString(rawString)
            ...
            } else if self.rawString.hasPrefix(Condition.ExpressionToken.Default.External.rawValue) {
                
                self.expression = (.Unspecified, .Unspecified, .Unspecified, 0)
                let externalKey = (rawString as NSString).substringFromIndex((Condition.ExpressionToken.Default.External.rawValue as NSString).length)
